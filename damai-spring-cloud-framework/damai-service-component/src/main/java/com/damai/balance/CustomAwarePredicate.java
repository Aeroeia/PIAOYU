...
public class CustomAwarePredicate extends AbstractServerPredicate{
	 ...
	 *
	 * 如果被调用服务的 spring.cloud.nacos.discovery.metadata.gray 配置项没有配置，或者为空，或者该配置项中的值不是true或false字符串(不区分大小写)则认为mark=false
	 * 判断逻辑:
    ...
    public boolean apply(PredicateKey input) {
		...
		try {
			...
			String grayFromRequest = null;
			if (ra != null) {
				ServletRequestAttributes sra = (ServletRequestAttributes) ra;
				HttpServletRequest request = sra.getRequest();
				grayFromRequest = request.getHeader(GRAY_PARAMETER);
			}
			if (StringUtils.isEmpty(grayFromRequest)) {
				grayFromRequest = BaseParameterHolder.getParameter(GRAY_PARAMETER);
			}
			if (StringUtils.isEmpty(grayFromRequest)) {
				grayFromRequest = gray;
			}
			NacosServer nacosServer = (NacosServer) input.getServer();
			Map<String, String> metadata = nacosServer.getInstance().getMetadata();
			String grayFromMetaData;
			if (CollectionUtil.isEmpty(metadata)) {
				grayFromMetaData = GRAY_FLAG_FALSE;
			}else {
				grayFromMetaData = metadata.get(GRAY_PARAMETER);
			}
			//判断如果被调用端没有灰度配置则默认配置为生产环境
			if(StringUtil.isEmpty(grayFromMetaData) || Objects.isNull(map.get(grayFromMetaData.toLowerCase()))) {
				grayFromMetaData = GRAY_FLAG_FALSE;
			}
			//判断如果请求端没有灰度标识则默认配置为生产环境
			if(StringUtil.isEmpty(grayFromRequest) || Objects.isNull(map.get(grayFromRequest.toLowerCase()))) {
				grayFromRequest = GRAY_FLAG_FALSE;
			}
			...
			if (result == false && grayFromRequest.equalsIgnoreCase(GRAY_FLAG_TRUE)) {
				List<Server> servers = Optional.ofNullable(customEnabledRule)
						...
						.orElseThrow(() -> new DaMaiFrameException(BaseCode.LOAD_BALANCER_NOT_EXIST));
				...
				for (Server server : servers) {
					...
					judgeList.add(balanceGray);
				}
				if(!judgeList.contains(GRAY_FLAG_TRUE)) {
					result = true;
				}
			}
		}catch (Exception e) {
